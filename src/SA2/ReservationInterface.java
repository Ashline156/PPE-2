...
public class ReservationInterface extends JFrame {
	...
	public void refreshTable() {
		try {
			...
			PreparedStatement reserStmt = con.prepareStatement(
					"Select id_reservation, nombres_personnes, dateArriver,  dateDepart, date_reservation, id_client, id_categorie, num_chambres from reservation");
	...
	public ReservationInterface() {
		...
		JLabel lblLabel = new JLabel("Gestion de Reservation");
		lblLabel.setForeground(Color.BLACK);
		lblLabel.setFont(new Font("Trebuchet MS", Font.BOLD, 29));
		lblLabel.setBounds(493, 26, 330, 29);
		contentPane.add(lblLabel);

		JLabel lbldateA = new JLabel("dateArriver");
		lbldateA.setForeground(Color.WHITE);
		lbldateA.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lbldateA.setBounds(12, 188, 195, 29);
		contentPane.add(lbldateA);

		JLabel lblidentifiant = new JLabel("id_reservation");
		lblidentifiant.setForeground(Color.WHITE);
		lblidentifiant.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lblidentifiant.setBounds(12, 131, 195, 29);
		contentPane.add(lblidentifiant);

		JLabel lbldateR = new JLabel("date_reservation");
		lbldateR.setForeground(Color.WHITE);
		lbldateR.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lbldateR.setBounds(12, 308, 195, 29);
		contentPane.add(lbldateR);

		JLabel lblCategoriechambre = new JLabel("id_categorie");
		lblCategoriechambre.setForeground(Color.WHITE);
		lblCategoriechambre.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lblCategoriechambre.setBounds(12, 543, 195, 29);
		contentPane.add(lblCategoriechambre);

		JLabel lbldateD = new JLabel("dateDepart");
		lbldateD.setForeground(Color.WHITE);
		lbldateD.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lbldateD.setBounds(12, 249, 195, 29);
...
//
//				if (nombres_personnes.trim().length() == 0 || dateArriver.trim().length() == 0
//						|| dateDepart.trim().length() == 0 || date_reservation.trim().length() == 0
//						|| statut.trim().length() == 0 || id_client.trim().length() == 0
//						|| id_categorie.trim().length() == 0 || num_chambres.trim().length() == 0) {
//
...
//								"INSERT INTO  reservation (nombres_personnes, dateArriver, dateDepart, date_reservation, id_client, id_categorie, num_chambres) VALUES(?,?,?,?,?,?,?)");
//
...
//		contentPane.add(btnEnregistrer);
		...
		btnSupprimer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (id.trim().length() == 0 || nombres_personnes.trim().length() == 0
						|| dateArriver.trim().length() == 0 || dateDepart.trim().length() == 0
						|| date_reservation.trim().length() == 0 || id_client.trim().length() == 0
						|| id_categorie.trim().length() == 0 || num_chambres.trim().length() == 0) {
		...
		btnSupprimer.setBounds(710, 697, 131, 29);
		contentPane.add(btnSupprimer);

		JLabel lblnombresP = new JLabel("nombres_personnes");
		lblnombresP.setForeground(Color.WHITE);
		lblnombresP.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		lblnombresP.setBounds(12, 371, 195, 29);
		contentPane.add(lblnombresP);
		...
		JLabel lblNumchambres = new JLabel("num_chambres");
...
//				System.out.println(num_chambres);
				...
				if (nombres_personnes.trim().length() == 0 || dateArriver.trim().length() == 0
					...
					try {
						...
						con = (Connection) db.db_connect();
						PreparedStatement stmt = con.prepareStatement(
								"INSERT INTO  reservation (nombres_personnes, dateArriver, dateDepart, date_reservation,statut, id_client, id_categorie, num_chambres) VALUES(?,?,?,?,?,?,?,?)");
		...
		btnModifier.setBounds(170, 697, 131, 29);
		...
		tableReser.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					Connection con;
					con = (Connection) db.db_connect();
					PreparedStatement callInfo = con
							.prepareStatement("SELECT * FROM reservation WHERE id_reservation='" + Clicktable + "'"); ////// faire
																														////// la
																														////// concatenation
																														////// avec
																														////// le
		...
		btnActualiser.setBackground(UIManager.getColor("InternalFrame.activeTitleGradient"));
		btnActualiser.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		btnActualiser.setBounds(1029, 142, 177, 29);
		contentPane.add(btnActualiser);

		JButton btnAnnuler = new JButton("Annuler");
		btnAnnuler.setBackground(Color.GREEN);
		btnAnnuler.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		btnAnnuler.setBounds(430, 697, 131, 29);
		...
		btnNouveau.setBackground(Color.BLUE);
		btnNouveau.setFont(new Font("Yu Gothic UI", Font.BOLD, 17));
		...
		btnNouveau.setBounds(985, 697, 141, 29);
		contentPane.add(btnNouveau);
		
		JLabel lblCritere = new JLabel("Critere");
		lblCritere.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblCritere.setBackground(Color.LIGHT_GRAY);
		lblCritere.setBounds(880, 117, 74, 16);
		contentPane.add(lblCritere);
		
		JLabel label = new JLabel("");
		label.setIcon(new ImageIcon("C:\\Users\\User\\Documents\\Eclipse\\SA2\\images\\reservation.jpg"));
		...
		JLabel label_1 = new JLabel("Recherche");
		label_1.setBounds(851, 100, 85, 16);
	...
	public static void showTable() {
		...
		try {
			...
			PreparedStatement userStmt = con.prepareStatement(
					"SELECT id_reservation AS '#',nombres_personnes as 'nombres_personnes', dateArriver as 'dateArriver',"
			...
			////////////// pour renomer le nom de colonne dans ma table en referant aux noms
			////////////// donnees dans la base de donnees======
